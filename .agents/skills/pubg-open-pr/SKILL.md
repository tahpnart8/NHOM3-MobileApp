...
4. **Check the style.** Run `sh scripts/check-java-style.sh` (skill `pubg-code`). Fix each finding on your lines, or say in the pull request why a line is better as it is. Confirm any lasting knowledge went into `memory/code/` and not into comments.
5. **Check scope.** `git diff --stat origin/main...HEAD`. Every changed file must be inside the issue's allowed packages or files. List anything outside and remove it, or ask.
6. **Check for deletions.** `git diff --diff-filter=DR --name-status origin/main...HEAD`. Any deleted or renamed file under `PUBGApp/app/src/main` needs an approved `type:removal` issue and a `Removal-Issue: #N` line in the body. Without one, restore the file.
7. **Check documents.** If a schema or a feature's behaviour changed, `docs/data/*` and `docs/product/features.md` are changed in this branch (AGENTS.md R7).
