...
- **Issues and pull requests are written in Vietnamese** (title and body), because people read them. The keywords `Closes`, `Refs`, `Removal-Issue` and `Deviation` and the `type(scope):` prefix stay English. CI rejects a pull request whose title or body is not Vietnamese (AGENTS.md R14).
